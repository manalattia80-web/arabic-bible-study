...
async function run() {
  const url = 'https://arabic-bible-study-production.up.railway.app/api/v1/verses?book_id=1&chapter_num=47';
  ...
  const userJsonText = fs.readFileSync('gen47.json', 'utf8');
  const userJson = JSON.parse(userJsonText);

  let result = [];
  let j = 0;
  
  if (userJson.length >= 2 && userJson[0].word === 'تكوين' && userJson[1].word === '47') {
      j = 2;
  }
  ...
  fs.writeFileSync('Gen47_fixed.json', JSON.stringify(validStrongs, null, 2));
  console.log("Successfully created Gen47_fixed.json with exactly", validStrongs.length, "words.");
}
run();
