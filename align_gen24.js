...
async function run() {
  ...
  const part1 = JSON.parse(fs.readFileSync('gen24_part.json', 'utf8'));
  const part2 = JSON.parse(fs.readFileSync('gen24_part2.json', 'utf8'));
  const part3 = JSON.parse(fs.readFileSync('gen24_part3.json', 'utf8'));
  
  const userJson = [...part1, ...part2, ...part3];
  
  // write the combined back for reference
  fs.writeFileSync('gen24_combined.json', JSON.stringify(userJson, null, 2));

  let result = [];
  let j = 0;
  
  if (userJson[0].word === 'تكوين' && userJson[1].word === '24') {
      j = 2;
  }
  ...
  fs.writeFileSync('Gen24_fixed.json', JSON.stringify(validStrongs, null, 2));
  console.log("Successfully created Gen24_fixed.json with exactly", validStrongs.length, "words.");
}
run();
