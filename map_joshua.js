...
3. If an Arabic word has no direct Hebrew equivalent in the source, assign "H0".
4. All Strong's numbers must start with 'H' (e.g., "H430").
...
${promptVerses}`;
    ...
    while(retries > 0) {
      try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash-lite:generateContent?key=${GEMINI_KEY}`, {
          method: 'POST',
        ...
        aiMappings = JSON.parse(textResponse);
        break;
      } catch (e) {
        ...
        let waitTime = 60000;
        const match = e.message && e.message.match ? e.message.match(/retry in ([\d\.]+)s/) : null;
        if (match) {
...
async function main() {
  ...
  // Book 6 is Joshua. It has 24 chapters.
  for (let ch = 7; ch <= 24; ch++) {
    await mapChapter(6, ch, 'Joshua');
    await new Promise(r => setTimeout(r, 20000)); // 20s between chapters
  }
  
  console.log("JOSHUA FINISHED!");
}
